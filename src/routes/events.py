...
async def get_interested(event_id: str, user: Annotated[User, Depends(get_current_user)], res: Response):
    ...
    try:
        interested = pg_interested.select(pg_interested.club_id, pg_interested.interestee, pg_club.club_name).join(pg_club).where(pg_interested.event_id == event_id).dicts()
    except DoesNotExist:
        res.status_code = 404
        return {"message": "Event not found"}
    try:
